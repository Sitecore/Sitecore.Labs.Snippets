...
RadEditorCommandList["InsertCustomSnippet"] = function (commandname, editor, tool) {

    var url = "/sitecore/shell/default.aspx?xmlcontrol=SnippetDialog&mode=rte";

    var selectedElement = editor.getSelectedElement();

   // if (selectedElement.tagName == "SPAN" || selectedElement.tagName == "FONT") {
        var field = selectedElement.getAttribute("field");
        var path = selectedElement.getAttribute("itemid");
        if (field != null && path != null) {       
            url += "&path=" + path + "&fields=|" + field + "|";
...
function scInsertSnippetCallBack (sender, returnValue){
    
    if (!returnValue || returnValue == undefined) {
        scEditor.pasteHtml("");
        return;
    }
   //scEditor.pasteHtml("[[" + returnValue.path + "," + returnValue.fields + "]]");
   //scEditor.pasteHtml("<span style='color:red' path='" + returnValue.path + "' field='" + returnValue.fields + "'>" +returnValue.fieldvalue+"</span> ");
